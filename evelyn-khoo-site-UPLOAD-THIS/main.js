(function () {
  ...
  if (grid) {
    ...
    // Live contributor count above the form (hidden if the fetch fails)
    var countWrap = document.getElementById('participate-count');
    var countText = document.getElementById('participate-count-text');
    if (countWrap && countText && window.fetch) {
      fetch('/.netlify/functions/results-data', { cache: 'no-store' })
        .then(function (res) { return res.ok ? res.json() : null; })
        .then(function (data) {
          if (data && data.total > 0) {
            countText.textContent =
              data.total + ' audit and controllership leaders have contributed — join them';
